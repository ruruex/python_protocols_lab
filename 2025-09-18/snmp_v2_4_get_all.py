...
def snmpv2_get_all(ip_address, community):
    ...
    # cpmCPUTotal5min, int
    # 实体5分钟平均的CPU使用率 取值范围：0～100 缺省值：0
    cpu_usage = int(asyncio.run(snmpv2_get(ip_address, community, "1.3.6.1.4.1.9.9.109.1.1.1.1.8.7",port=161))[1])
    
    # print(cpu_usage)
    # cpmCPUMemoryUsed, int
    mem_used = int(asyncio.run(snmpv2_get(ip_address,community,"1.3.6.1.4.1.9.9.109.1.1.1.1.12.7",  port=161))[1])
    # cpmCPUMemoryFree, int
    mem_free = int(asyncio.run(snmpv2_get(ip_address, community, "1.3.6.1.4.1.9.9.109.1.1.1.1.13.7", port=161))[1])

    # memory utilization in percenage, float
    mem_percent = float(round((mem_used / (mem_used + mem_free)*100), 4))
    ...
    # 获取接口管理状态, list of int, 1 is up, 0 is down
    ...
    interface_list = []
    for name, speed, status, in_bytes, out_bytes in zip(if_name_list, if_speed_list, if_status_list, if_in_bytes_list, if_out_bytes_list): # zip as concated list
        #print(name,speed, status, in_bytes, out_bytes)
